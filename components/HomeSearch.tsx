...
export default function HomeSearch() {
  ...
  const [results, setResults] = useState<{ providers: any[]; books: any[]; courses: any[] } | null>(null) // Fixed type
  const [loading, setLoading] = useState(false)
  const router = useRouter()

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (query.trim()) { router.push(`/providers?search=${encodeURIComponent(query.trim())}`) }
  }

  useEffect(() => {
    const fetchResults = async () => {
      if (query.trim().length < 1) { setResults(null); setLoading(false); return }
      setLoading(true)
      ...
      try {
        ...
        const filteredProviders = (providersRes.data || []).filter((p: any) => Object.values(p).some((val: any) => String(val || '').toLowerCase().includes(q))).sort((a: any, b: any) => {
        ...
        const filteredBooks = (booksRes.data || []).filter((b: any) => Object.values(b).some((val: any) => String(val || '').toLowerCase().includes(q))).slice(0, 3)
        const filteredCourses = (coursesRes.data || []).filter((c: any) => Object.values(c).some((val: any) => String(val || '').toLowerCase().includes(q))).slice(0, 3)

        setResults({ providers: filteredProviders, books: filteredBooks, courses: filteredCourses })
  ...
  const quickSearch = (term: string) => setQuery(term)
  const hasResults = results && (results.providers.length > 0 || results.books.length > 0 || results.courses.length > 0)
  ...
  return (
    <div className="relative max-w-3xl mx-auto">
      <form onSubmit={handleSearchSubmit} className="relative flex items-center bg-white border border-black/10 rounded-full shadow-[0_20px_60px_-20px_rgba(0,0,0,0.15)] focus-within:border-primary focus-within:shadow-[0_20px_60px_-20px_rgba(193,18,31,0.25)] transition-all overflow-hidden">
        ...
        <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search providers, books, courses..." className="w-full bg-transparent pl-16 pr-4 py-6 text-lg focus:outline-none" />
        <div className="pr-2"><button type="submit" className="btn-primary !py-3.5 !px-6 text-sm whitespace-nowrap"><span>Search</span></button></div>
      </form>
            ...
            <div className="p-3">
              ...
              {results!.providers.map((p: any) => (
                <Link href={`/providers/${p.id}`} key={p.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-paper transition cursor-pointer">
                  <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white font-semibold text-sm">{p.full_name?.charAt(0) || p.name?.charAt(0) || 'P'}</div>
            ...
            <div className="p-3 border-t border-black/5">
              ...
              {results!.books.map((b: any) => (
                <Link href={`/books`} key={b.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-paper transition cursor-pointer">
                  <div className="w-8 h-10 rounded bg-ink flex items-center justify-center text-white text-xs"><i className="fas fa-book"></i></div>
                  <div className="flex-1"><div className="text-sm font-medium">{b.title}</div><div className="text-xs text-muted">by {b.author}</div></div>
                  <i className="fas fa-arrow-right text-xs text-muted"></i>
                </Link>
              ))}
            </div>
            ...
            <div className="p-3 border-t border-black/5">
              ...
              {results!.courses.map((c: any) => (
                <Link href={`/courses`} key={c.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-paper transition cursor-pointer">
                  <div className="w-10 h-10 rounded-lg bg-accent/20 flex items-center justify-center text-accent"><i className="fas fa-graduation-cap text-sm"></i></div>
                  <div className="flex-1"><div className="text-sm font-medium">{c.title}</div><div className="text-xs text-muted">{c.instructor} {c.price ? `• ₦${c.price.toLocaleString()}` : ''}</div></div>
                  <i className="fas fa-arrow-right text-xs text-muted"></i>
                </Link>
              ))}
            </div>
