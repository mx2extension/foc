...
export default function AboutBookPage() {
  ...
  useEffect(() => {
    ...
    if (storedReader) {
      try { setReader(JSON.parse(storedReader)) } catch (e) {}
    }
  }, [])
  ...
  useEffect(() => {
    ...
    const checkOwnership = async () => {
      const { data } = await supabase.from('book_purchases').select('id').eq('reader_email', reader.email.toLowerCase()).eq('book_id', book.id)
      if (data && data.length > 0) setIsOwned(true)
    }
    checkOwnership()
  }, [reader, book])
  ...
  const handleBuy = async () => {
    if (!reader) { 
      showToast('Please log in to the campus first.'); 
      window.location.href = `/login?redirect=/books/${book.id}`;
      return; 
    }
  ...
  return (
    <div className="relative overflow-hidden py-32 px-6 lg:px-10">
        ...
        <div className="fixed inset-0 bg-black/90 z-[200] flex flex-col p-2 md:p-6">
          <div className="flex justify-between items-center mb-2 md:mb-4 px-2">
            ...
            <div className="flex gap-2">
              <a href={book.download_url} download className="btn-secondary !py-2 !px-4 text-xs md:text-sm">
      ...
      <div className="max-w-4xl mx-auto">
        <Link href="/books" className="mb-12 inline-flex items-center gap-2 text-sm text-muted hover:text-primary transition">
          <i className="fas fa-arrow-left text-xs"></i> Back to Bookstore
        </Link>

        <div className="grid md:grid-cols-3 gap-12">
          <div className="md:col-span-1">
            <div 
              className="aspect-[3/4] rounded-xl relative overflow-hidden shadow-2xl flex flex-col justify-between p-8 text-white sticky top-32"
          ...
          <div className="md:col-span-2">
            ...
            <div className="flex items-center gap-4 mb-10">
              {book.price === 0 ? (
                ...
                <span className="px-4 py-2 rounded-full bg-primary/10 text-primary font-bold text-sm">₦{book.price.toLocaleString()}</span>
              )}
            ...
            {book.about && (
              <div className="premium-card p-8 mb-10">
                <h2 className="serif text-2xl mb-4">About the Book</h2>
            ...
            <div className="border-t border-black/5 pt-8">
              ...
              <div className="flex items-center gap-3">
                <a href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareText)}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-ink hover:text-white transition">
                  <i className="fab fa-twitter"></i>
                </a>
                <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-ink hover:text-white transition">
                  <i className="fab fa-facebook-f"></i>
                </a>
                <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-ink hover:text-white transition">
                  <i className="fab fa-linkedin-in"></i>
                </a>
                <a href={`https://wa.me/?text=${encodeURIComponent(shareText + ' ' + shareUrl)}`} target="_blank" rel="noopener noreferrer" className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-[#25D366] hover:text-white transition">
                  <i className="fab fa-whatsapp"></i>
                </a>
                <button onClick={() => { navigator.clipboard.writeText(shareUrl); showToast('Link copied!') }} className="w-10 h-10 rounded-full border border-black/10 flex items-center justify-center hover:bg-ink hover:text-white transition">
