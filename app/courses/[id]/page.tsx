...
export default function AboutCoursePage() {
  ...
  useEffect(() => {
    ...
    const checkEnrollment = async () => {
      const { data } = await supabase.from('course_enrollments').select('id').eq('reader_email', reader.email.toLowerCase()).eq('course_id', course.id)
  ...
  const handleBuy = async () => {
    ...
    if (!paystackKey || !(window as any).PaystackPop) {
      window.dispatchEvent(new CustomEvent('show-fallback-payment', { 
        detail: { amount: course.price.toLocaleString(), description: `${course.title} (Course)` }
      }))
      return; 
    }
      ...
      const handler = (window as any).PaystackPop.setup({
        ...
        callback: function(response: any) {
          const verifyPayment = async () => {
            ...
            setIsEnrolled(true)
            showToast('Enrollment successful! You can now start learning.')
          }
          verifyPayment()
        },
  ...
  return (
    <div className="relative overflow-hidden py-32 px-6 lg:px-10">
      ...
      <div className="max-w-5xl mx-auto">
        <Link href="/courses" className="mb-12 inline-flex items-center gap-2 text-sm text-muted hover:text-primary transition">
          <i className="fas fa-arrow-left text-xs"></i> Back to Courses
        </Link>
          ...
          <div className="md:col-span-1">
            <div className="premium-card p-8 sticky top-32">
              ...
              {isEnrolled || course.price === 0 ? (
                <div className="bg-green-50 text-green-800 p-4 rounded-xl text-sm font-medium text-center">
                  <i className="fas fa-check-circle mr-2"></i> You are enrolled
                </div>
              ) : (
                <button onClick={handleBuy} disabled={paying} className="btn-primary w-full justify-center !py-4 text-sm disabled:opacity-50">
                  {paying ? 'Processing...' : <><i className="fas fa-cart-shopping"></i> Enroll Now</>}
                </button>
              )}
            </div>
          </div>
