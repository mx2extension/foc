...
export default function CoursesPage() {
  ...
  const handleLogout = () => {
    ...
    setReader(null); setEnrolledCourseIds([]); setCourses([]); setActiveTab('all')
  }
        ...
        {loading ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                <div key={course.id} className="premium-card p-8 flex flex-col">
                  <div className="flex items-start justify-between mb-6">
                    ...
                    <div className="text-right">
                      ...
                      <div className="serif text-2xl">{course.price === 0 ? 'Free' : `₦${course.price.toLocaleString()}`}</div>
                    </div>
                  </div>
                  ...
                  <div className="mt-auto space-y-2">
                    {isEnrolled || isFree ? (
                      <Link href={`/courses/${course.id}`} className="btn-primary w-full justify-center !py-3 text-sm">
                        <i className="fas fa-play"></i> Start Course
                      </Link>
                    ) : (
                      <div className="flex gap-2">
                        <button onClick={() => handleBuyCourse(course)} className="flex-1 btn-primary justify-center !py-3 text-sm">
                          <i className="fas fa-cart-shopping text-xs"></i> Enroll
                        </button>
                        <Link href={`/courses/${course.id}`} className="flex-1 btn-secondary justify-center !py-3 text-sm !bg-paper">
            ...
            {displayedCourses.length === 0 && (
              <div className="col-span-full text-center py-20 text-muted">No courses available.</div>
            )}
          </div>
        )}
      </div>
