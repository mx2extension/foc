...
export default function ProviderDashboard() {
  ...
  useEffect(() => {
    ...
    let providerData
    try {
      providerData = JSON.parse(storedProvider)
    } catch (e) {
    ...
    const fetchProvider = async () => {
      ...
      if (data) {
        if (data.membership === 'pro' && data.membership_expires_at) {
          const expiryDate = new Date(data.membership_expires_at)
          if (expiryDate < new Date()) {
            ...
            showToast('Your Pro Membership has expired. Renew to keep your perks.', 'info')
          }
        }
  ...
  const handleProUpgrade = async () => {
    ...
    if (!paystackKey || typeof window === 'undefined' || !(window as any).PaystackPop) {
      window.dispatchEvent(new CustomEvent('show-fallback-payment', { 
        detail: { 
          ...
          description: `Pro Membership (${selectedPlan.months} month(s))` 
        } 
      }))
      return;
    }
    ...
    try {
      const handler = (window as any).PaystackPop.setup({
        key: paystackKey,
        email: provider.email || 'guest@findonecampus.com',
      ...
      showToast('Something went wrong while trying to open the payment window.', 'error')
      setUpgrading(false)
    }
  }
  ...
  const handleSocialClick = (e: React.MouseEvent) => {
    if (provider.membership !== 'pro') {
      e.preventDefault()
      showToast('Upgrade to Pro to add social links to your profile.', 'info')
    }
  }
    ...
    <div className="max-w-4xl mx-auto py-32 px-6">
      ...
      {activeBook && (
        <div className="fixed inset-0 bg-black/80 z-[200] flex flex-col p-4 md:p-8">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={() => setActiveBook(null)} className="text-white w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center transition">
              <i className="fas fa-times text-xl"></i>
            </button>
          </div>
          <div className="flex-1 bg-white rounded-2xl overflow-hidden">
            <iframe src={activeBook.file_url} className="w-full h-full" title={activeBook.title}></iframe>
          </div>
        </div>
      )}
      ...
      {toast && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[100] animate-[slideUp_0.3s_ease]">
          <div className={`flex items-center gap-3 px-6 py-4 rounded-full shadow-2xl text-white text-sm font-medium ${
            toast.type === 'success' ? 'bg-green-600' : toast.type === 'error' ? 'bg-primary' : 'bg-ink'
          }`}>
            <i className={`fas ${
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              <span className={`px-3 py-1.5 rounded-full text-sm font-medium ${provider.is_approved ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                {provider.is_approved ? 'Approved' : 'Pending Approval'}
              </span>
              <span className={`px-3 py-1.5 rounded-full text-sm font-medium ${provider.verification_status === 'verified' ? 'bg-blue-100 text-blue-800' : 'bg-gray-200 text-gray-800'}`}>
                Verification: {provider.verification_status.replace(/_/g, ' ')}
              </span>
            </div>
          </div>
            ...
            {provider.membership === 'pro' ? (
              <div>
                ...
                {provider.membership_expires_at && (
                  <p className="text-xs text-muted mt-2">
                    ...
                    Expires on: {new Date(provider.membership_expires_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                  </p>
                )}
              </div>
            ) : (
              <div>
                ...
                <p className="text-sm text-muted mb-4">
                  Upgrade to Pro to unlock social links, rank higher in search, and access the Pro Book Library.
                </p>
                
                {/* Plan Selector */}
                <div className="flex gap-2 mb-4">
                  <button 
                    onClick={() => setSelectedPlan({ months: 1, price: 7000 })}
                    className={`flex-1 py-2 rounded-xl text-xs font-medium transition ${selectedPlan.months === 1 ? 'bg-ink text-white' : 'bg-white border border-black/5 text-muted'}`}
                  >
                    1 Month<br/>₦7,000
                  </button>
                  <button 
                    onClick={() => setSelectedPlan({ months: 3, price: 18000 })}
                    className={`flex-1 py-2 rounded-xl text-xs font-medium transition ${selectedPlan.months === 3 ? 'bg-ink text-white' : 'bg-white border border-black/5 text-muted'}`}
                  >
                    3 Months<br/>₦18,000
                  </button>
                  <button 
                    onClick={() => setSelectedPlan({ months: 12, price: 60000 })}
                    className={`flex-1 py-2 rounded-xl text-xs font-medium transition ${selectedPlan.months === 12 ? 'bg-ink text-white' : 'bg-white border border-black/5 text-muted'}`}
                  >
                    1 Year<br/>₦60,000
                  </button>
                </div>
      ...
      {provider.membership === 'pro' && (
        <div className="premium-card p-8 mb-8">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h2 className="text-xl font-semibold">Pro Member Library</h2>
          </div>
          <div className="space-y-4">
            {proBooks.length === 0 ? (
              <p className="text-muted text-sm">No books in the library yet. Check back soon!</p>
            ) : (
              proBooks.map((book: any) => (
                <div key={book.id} className="flex flex-col md:flex-row md:items-center justify-between p-4 bg-paper rounded-xl border border-black/5 gap-4">
                  <div className="flex items-center gap-4">
                    {book.cover_config?.image_url ? (
                      <img src={book.cover_config.image_url} alt={book.title} className="w-12 h-16 rounded-md object-cover shadow-md" />
                    ) : (
                      <div className="w-12 h-16 rounded-md shadow-md flex items-center justify-center text-white" style={{ background: book.cover_config?.bg || 'linear-gradient(135deg, #1A1A1A, #C1121F)' }}>
                        <i className="fas fa-book-bookmark text-lg"></i>
                      </div>
                    )}
                    <div>
                  ...
                  >
                    <i className="fas fa-eye"></i> Read Book
                  </button>
                </div>
              ))
            )}
      ...
      <form onSubmit={handleUpdate} className="premium-card p-8 space-y-6">
        <div className="grid sm:grid-cols-3 gap-5">
          ...
          <div>
            ...
            <input value={form.profession || ''} onChange={e => setForm({...form, profession: e.target.value})} className="form-input" />
          </div>
        </div>

        <div>
          <label className="text-sm font-medium mb-2 block">Short Bio</label>
          <textarea rows={4} value={form.bio || ''} onChange={e => setForm({...form, bio: e.target.value})} className="form-input"></textarea>
        </div>

        <div>
          <label className="text-sm font-medium mb-2 block">Skills (comma separated)</label>
          <input value={form.skills || ''} onChange={e => setForm({...form, skills: e.target.value})} className="form-input" />
        </div>

        <div className="grid sm:grid-cols-3 gap-5">
          <div>
            <label className="text-sm font-medium mb-2 block">WhatsApp</label>
            <input value={form.whatsapp || ''} onChange={e => setForm({...form, whatsapp: e.target.value})} className="form-input" />
          </div>
          <div>
            <label className="text-sm font-medium mb-2 block">Country</label>
            <input value={form.country || ''} onChange={e => setForm({...form, country: e.target.value})} className="form-input" />
          </div>
          <div>
        ...
        <div className="pt-4 border-t border-black/5">
          <div className="flex items-center justify-between mb-4">
            ...
            {provider.membership !== 'pro' && (
              <span className="text-[10px] px-2 py-1 bg-accent/15 text-accent rounded-full font-bold uppercase">Pro Only</span>
            )}
          </div>
          <div className="space-y-4">
            <input 
