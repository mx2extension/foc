...
export default function ArticlePage() {
  ...
  useEffect(() => {
    ...
    const fetchArticle = async () => {
      const { data } = await supabase.from('news_updates').select('*').eq('id', articleId).single()
      setArticle(data)
    }
    fetchArticle()
  }, [articleId])
  ...
  return (
    <div className="py-32 relative">
      ...
      <article className="relative max-w-3xl mx-auto px-6 lg:px-10 z-10">
        ...
        {article.image_url && (<div className="aspect-video w-full rounded-3xl overflow-hidden mb-12 shadow-xl"><img src={article.image_url} alt={article.title} className="w-full h-full object-cover" /></div>)}
